    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _userName = userInfo['nama'] ?? userInfo['name'] ?? 'Tenant';
          _tenantId = userInfo['id_tenant']?.toString();
        });

        print('User: $_userName, Tenant ID: $_tenantId');
      }
    } catch (e) {
      print('Error loading user info: $e');
      if (mounted) {
        setState(() {
          _errorMessage = 'Gagal memuat info user: ${e.toString()}';
          _userName = 'Error';
        });
      }
    }
                      ...
                      InkWell(
                        onTap: () async {
                          final result = await Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => TambahKategoriScreen(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (_searchController.text.isEmpty) ...[
                      const SizedBox(height: 8),
                      Text(
                        'Tap tombol "Tambah Menu" untuk mulai',
                        style: TextStyle(
                          fontSize: 14,
